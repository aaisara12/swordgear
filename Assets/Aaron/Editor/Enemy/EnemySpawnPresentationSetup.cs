...
public static class EnemySpawnPresentationSetup
{
    ...
    public static void WireFromMenu()
    {
        ...
        foreach (string prefabName in EnemyPrefabNames)
        {
            ...
            if (WireEnemyPrefab(path, controller, auraPrefab))
            {
                wired++;
            }
        }
        ...
        Debug.Log(
            ...
            $"Anim={ControllerPath}, Aura={AuraPrefabPath}, EliteSO={ElitePresentationPath}.");
    }
    ...
    private static AnimationClip CreateOrUpdateSpawnClip()
    {
        ...
        // VisualRoot localScale: 0 → overshoot → settle at 1 over 0.65s
        AnimationCurve scaleXY = new AnimationCurve(
            new Keyframe(0f, 0f),
            new Keyframe(0.18f, 1.15f),
            new Keyframe(0.45f, 0.96f),
            new Keyframe(0.65f, 1f));
        AnimationCurve scaleZ = AnimationCurve.Constant(0f, 0.65f, 1f);
    ...
    private static GameObject CreateOrUpdateEliteAuraPrefab()
    {
        ...
        try
        {
            ...
            main.startColor = new Color(1f, 0.75f, 0.2f, 0.85f);
    ...
    private static bool WireEnemyPrefab(string prefabPath, AnimatorController controller, GameObject auraPrefab)
    {
        ...
        try
        {
            ...
            Transform? aura = root.transform.Find("EliteAura");
            if (aura == null)
            {
            ...
            serialized.FindProperty("spawnDurationFallback").floatValue = 0.65f;
            serialized.FindProperty("eliteAuraChild").objectReferenceValue = aura != null ? aura.gameObject : null;
    ...
    private static Transform EnsureVisualRoot(GameObject root)
    {
        ...
        for (int i = 0; i < root.transform.childCount; i++)
        {
            ...
            if (child.name == "VisualRoot" || child.name == "EliteAura")
            {
                continue;
            }
